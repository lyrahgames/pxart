...
static inline double benchmark_performance(const size_t samples) {
  ...
  const auto performance = samples * 4 / time / (1024.0 * 1024.0);
  return performance;
}
...
#define ADD(X) add(#X, benchmark_performance<X>)
  ...
  friend ostream& operator<<(ostream& os, benchmark& d) {
    os << setw(30) << "Performance Benchmark" << setw(30)
       << "samples/run = " << d.n << ", runs = " << d.run_count << '\n'
       << string(105, '-') << '\n'
       << left << setw(30) << "Name" << right << setw(25) << "Max. [MiB/s]"
       << setw(25) << "Mean [MiB/s]" << setw(25) << "Stddev. [MiB/s]\n"
       << string(105, '-') << '\n';
...
int main(int argc, char** argv) {
  ...
  plot << "set xr [-1:" << driver.runners.size() << "]\n"
       << "set title \"Performance Benchmark (samples/run = " << driver.n
