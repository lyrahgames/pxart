...
// void set_rng(std::string name, RNG&& rng, bool reverse_bits = false)
// noexcept; template <typename RNG,
//           typename std::enable_if_t<
//               std::is_default_constructible_v<std::decay_t<RNG>>, int> = 0>
// void set_rng(std::string name, RNG&& rng, bool reverse_bits = false)
// noexcept; template <typename RNG> void set_rng(RNG&& rng, bool reverse_bits =
// false) noexcept;
