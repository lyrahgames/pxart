...
\label{sec:background}
  ...
  \label{sub:pseudorandom_number_generators}
    ...
    Since the set of states in a PRNG is finite, every respective pseudorandom sequence has to be ultimately periodic \autocite{lecuyer1994}.
    \begin{lemma}[Pseudorandom Sequences are Ultimately Periodic]
      Let $\mathscr{G}\define (S,T,U,G)$ be a PRNG and $s_0\in S$ its initial state.
      Then the respective pseudorandom sequence $(u_n)_{n\in\setNatural}$ is ultimately periodic, that is there exist $ρ,τ\in\setNatural$ with $ρ\leq \# S$ such that for all $n\in\setNatural$ with $n\geq τ$ the following holds.
      \[
        u_{n+ρ} = u_n
      \]
      We call ρ the period and τ the transient.
    \end{lemma}
    ...
    The following paragraphs will provide concrete examples of an application of this definition to deepen the understanding.
