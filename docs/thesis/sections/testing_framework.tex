...
\label{sec:testing_framework}
  ...
  \label{sub:generation_benchmark}
    ...
    To apply the benchmarking routine easily to any possible generator, we use an abstraction class, given in listing \ref{code:generation-benchmark-structure}, which saves the context variables, such as the number of samples and the \citetitle{perfevent} parameters.
